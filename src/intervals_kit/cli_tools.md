...
installation. Use it as-is for all CLI calls.

When installed from PyPI (standard):
```bash
uvx --from intervals-kit intervals-icu-tools <command>
```

When running from a local source tree (development):
```bash
...
Or create `~/.config/intervals-kit/config.toml`:
```toml
...
- `-o / --output-dir <path>`: Directory **or full file path** to save output (default: `.` = stdout).
  - Directory: `uvx --from intervals-kit intervals-icu-tools -o ./data list-activities …` → saves to `./data/activities_<dates>.json`
  - File path: `uvx --from intervals-kit intervals-icu-tools -o ./data/my_activities.json list-activities …` → saves to exactly that path
...
```
uvx --from intervals-kit intervals-icu-tools -o ./data list-activities --oldest 2024-01-01 --newest 2024-12-31
#                          ^^^^^^^^^^^ before subcommand ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] list-activities --oldest 2024-01-01 --newest 2024-12-31 [--limit 50]
```
...
```
uvx --from intervals-kit intervals-icu-tools get-activity <activity_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] search-activities "<query>" [--full] [--limit 20]
```
...
```
uvx --from intervals-kit intervals-icu-tools update-activity <activity_id> [--name "New Name"] [--description "..."] [--type Run] [--perceived-exertion 7]
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-intervals <activity_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-streams <activity_id> [--types watts,heartrate,cadence]
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-power-curve <activity_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-hr-curve <activity_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-pace-curve <activity_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-best-efforts <activity_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-segments <activity_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools -o ./data download-activity-file <activity_id> --type original|fit|gpx
```
...
```
uvx --from intervals-kit intervals-icu-tools -o ./data download-activities-csv --oldest 2024-01-01 --newest 2024-12-31
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-athlete-power-curves --oldest 2024-01-01 --newest 2024-12-31
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-athlete-hr-curves --oldest 2024-01-01 --newest 2024-12-31
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-athlete-pace-curves --oldest 2024-01-01 --newest 2024-12-31
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] get-athlete
```
...
```
uvx --from intervals-kit intervals-icu-tools update-athlete --fields '{"weight": 70.5, "timezone": "Europe/Berlin"}'
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] list-wellness --oldest 2024-01-01 --newest 2024-12-31
```
...
```
uvx --from intervals-kit intervals-icu-tools get-wellness 2024-01-15
```
...
```
uvx --from intervals-kit intervals-icu-tools update-wellness 2024-01-15 --fields '{"weight": 70.2, "sleepSecs": 28800, "fatigue": 3}'
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] list-events --oldest 2024-01-01 --newest 2024-12-31 [--category WORKOUT,RACE_A]
```
...
```
uvx --from intervals-kit intervals-icu-tools get-event <event_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools create-event --fields '{"name": "Long Run", "category": "WORKOUT", "start_date_local": "2024-03-15T09:00:00", "type": "Run", "moving_time": 5400}'
```
...
```
uvx --from intervals-kit intervals-icu-tools update-event <event_id> --fields '{"name": "Recovery Ride", "moving_time": 3600}'
```
...
```
uvx --from intervals-kit intervals-icu-tools delete-event <event_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] list-workouts
```
...
```
uvx --from intervals-kit intervals-icu-tools get-workout <workout_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools create-workout --fields '{"name": "Tempo Run", "type": "Run", "moving_time": 3600}'
```
...
```
uvx --from intervals-kit intervals-icu-tools update-workout <workout_id> --fields '{"name": "Updated Name"}'
```
...
```
uvx --from intervals-kit intervals-icu-tools delete-workout <workout_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools [-o ./data] list-messages <activity_id>
```
...
```
uvx --from intervals-kit intervals-icu-tools create-message <activity_id> "Great workout!"
```
...
2. For longer activities, use CLI to save to disk:
   ```
   uvx --from intervals-kit intervals-icu-tools -o ./data get-streams <id> --types watts,heartrate
   ```
...
uvx --from intervals-kit intervals-icu-tools get-power-curve <id>
```
